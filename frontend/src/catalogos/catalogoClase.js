...
const CatalogoClase = props => {
            ...
            fn: items => {
                if (target.value === "") {
                    ...
                    return items.filter(x => {
                         if ( String(x.clase_id).includes(target.value)
                            ||x.clase_nombre.toLowerCase().includes(target.value.toLowerCase())
                           ) {
                            return x;
                        } 
                    });
    ...
    const seleccionarClase = (index) => (event) => {
        ...
        var nom = recordsAfterPagingAndSorting()[index].clase_nombre;
    ...
    return (
        ...
        >
            <Modal.Header className='bg-danger'>
                <Modal.Title style={{ color: "#fff" }}>Seleccionar Clase Vehiculo.</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                        ...
                        <TableBody>
                            {
                                records && recordsAfterPagingAndSorting().map((item, index) => (
                                    <TableRow key={item.idclaseminas} style={{ padding: "0" }} onClick={seleccionarClase(index)}>
                                        <TableCell style={{ fontSize: "10px", textAlign: "center", verticalAlign: "middle" }}>{item.clase_id}</TableCell>
                                        <TableCell style={{ fontSize: "10px", textAlign: "center", verticalAlign: "middle" }}>{item.clase_nombre}</TableCell>
