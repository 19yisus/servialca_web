      ...
      fn: items => {
        if (target.value === "")
          ...
          return items.filter(x => {
            if ((x.usoVehiculo_id !== null ? String(x.usoVehiculo_id).includes(target.value) : 0)
              || (x.usoVehiculo_nombre !== null ? x.usoVehiculo_nombre.toLowerCase().includes(target.value.toLowerCase()) : '')
