...
import { ModalDebito } from './modalGestionDebito';
import moment from 'moment'

const NotasdeDebito = (props) => {
      ...
      fn: items => {
        if (target.value === "")
          ...
          return items.filter(x => {
            console.log(x.ced)
            if (    (x.conceptodebito !== null ? x.conceptodebito.toLowerCase().includes(target.value.toLowerCase()) : '')
            ...
            <div className='col-md-12 mx-auto row'>
              <div className='col-md-12 mx-auto d-flex justify-content-end px-3'>
                <div className='col-md-5 row'>
                  <div className='card bg-danger py-3 d-flex justify-content-end'>
              ...
              <div className='col-md-12 mx-auto d-flex justify-content-center px-3 mb-4'>
                <h3 className='text-dark'>Notas de Debito</h3>
              </div>
             ...
             <TableBody >
               {
                 records && recordsAfterPagingAndSorting().map((item, index) => (
                   <TableRow key={index} style={{ padding: "0" }}>
                     ...
                     <TableCell className='align-baseline' style={{ textAlign: "center", alignItems: 'center', width: '200px' }}>{item.conceptodebito}</TableCell>
                     <TableCell className='align-baseline' style={{ textAlign: "right", alignItems: 'center', width: '200px' }}><span className='text-danger'>{ '- '+ item.montodebito + ' Bs.'}</span></TableCell>

                    

                   {item.reversado !== '1' ? 
                    <TableCell className='align-baseline' style={{ textAlign: "center", alignItems: 'center' }}>
                    <button onClick={seleccionaOperacion(item.idnotadebito, 3)}  className="btn btn-sm mx-1 btn-info rounded-circle" ><i className="fas fas fa-backward"></i> </button>
                    <button onClick={seleccionaOperacion(item.idnotadebito, 2)}  className="btn btn-sm mx-1 btn-warning rounded-circle"><i className="fa fa-edit"></i> </button>
                  </TableCell>
                  : 
                  <TableCell className='align-baseline' style={{ textAlign: "center", alignItems: 'center' }}>
